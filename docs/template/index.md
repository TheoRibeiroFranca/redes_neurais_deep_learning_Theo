...
O `mkdocs.yml` já aponta para este repositório — não há mais nada a trocar:

``` { .yaml title="mkdocs.yml" }
site_name: ANN-DL · Entregas — Theo Ribeiro França
site_author: Theo Ribeiro França
site_url: https://theoribeirofranca.github.io/redes_neurais_deep_learning_Theo
repo_url: https://github.com/TheoRibeiroFranca/redes_neurais_deep_learning_Theo
repo_name: TheoRibeiroFranca/redes_neurais_deep_learning_Theo
```

O `site_url` precisa ser **idêntico** à URL que aparece em **Settings → Pages**: é dele que
o Material monta os links do menu e o `sitemap.xml`. A URL do Colab, no fim da `nav`, aponta
para o notebook deste repositório pelo mesmo motivo — um endereço de outra pessoa abriria o
notebook dela.
...
`PYTHON_VERSION`, no [workflow](https://github.com/TheoRibeiroFranca/redes_neurais_deep_learning_Theo/blob/main/.github/workflows/main.yaml) —
...
Instale as dependências com o `requirements-ml.txt` — ele já inclui o
`requirements.txt` (MkDocs e plugins) e acrescenta as bibliotecas das entregas
(numpy, pandas, matplotlib/seaborn, scikit-learn, umap-learn, torch):
...
=== "Windows"
    ...
    ``` shell
    python -m pip install -r requirements-ml.txt --upgrade
    ```
...
- [ ] Nenhum resquício de template: `grep -rn "TROCAR\|usuario/ann-dl\|Seu Nome" mkdocs.yml docs/`
