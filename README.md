...
São dois arquivos de dependências:

| Arquivo | O que traz | Quem instala |
|---|---|---|
| [requirements.txt](requirements.txt) | MkDocs e plugins do site | o CI, a cada push |
| [requirements-ml.txt](requirements-ml.txt) | inclui o de cima **+** seaborn, scipy, umap-learn, torch/torchvision (CPU), tqdm, ipywidgets | você, no `env/` |

O CI não precisa das bibliotecas das entregas: o site é construído com
`execute: false` e publica as saídas já salvas nos notebooks. Manter torch fora do
`requirements.txt` é o que faz cada build do site levar segundos em vez de minutos.

O `torch` está fixado nas wheels `+cpu` porque esta máquina não tem GPU NVIDIA — veja o
comentário no `requirements-ml.txt` antes de rodar em uma máquina com GPU.
...
2. Nada a trocar no [mkdocs.yml](mkdocs.yml): `site_url`, `repo_url` e `repo_name` já
   apontam para este repositório.
